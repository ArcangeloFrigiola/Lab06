...
public class Model {
	...
	public String getUmiditaMedia(int mese) {
		Map<Citta, Double> temp = new HashMap<>(dao.getUmiditaMediaMese(mese));
		String result = "Umidità media:\n";

		for (Citta c : temp.keySet()) {
			result += c.getNome() + ": " + temp.get(c) + "\n";
		}
		return result;
	}
	 ...
	 */
	public String calcolaSequenza(Month mese) {
		...
		String result = "Best soluzione:\n";
		for(Citta r: best) {
			result+=r.getNome()+"\n";
		}
		result+= "Costo totale: "+bestCosto+"\n";
		
		for(Citta c1: leCitta) {
